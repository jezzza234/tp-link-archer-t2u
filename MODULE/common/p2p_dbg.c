...

{
	...
	pP2PCtrl->bUsePeerChannel = ((SigmaEnabled == 0) ? USE_PEER_CHANNEL_DEFAULT : FALSE);
...
INT Set_P2P_UsePeerChannel_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	POS_COOKIE			pObj;
	...
	if (simple_strtol(arg, 0, 10) == 0)
		pP2PCtrl->bUsePeerChannel = FALSE;
	else
		pP2PCtrl->bUsePeerChannel = TRUE;
	
	DBGPRINT(RT_DEBUG_ERROR, ("%s:: bUsePeerChannel = %d\n", __FUNCTION__, pP2PCtrl->bUsePeerChannel));
	return TRUE;
}
