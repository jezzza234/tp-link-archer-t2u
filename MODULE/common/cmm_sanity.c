    ...
    while ((Length + 2 + pEid->Len) <= MsgLen)    
    {
		...
		break;
            ...
            case IE_TIM:
                ...
                {
#ifdef P2P_SUPPORT
                    if (P2P_CLI_ON(pAd) && NdisEqualMemory(pBssid, pAd->ApCfg.ApCliTab[0].ApCliMlmeAux.Bssid, MAC_ADDR_LEN))
                    {
                        GetTimBit((PCHAR)pEid, pAd->ApCfg.ApCliTab[0].ApCliMlmeAux.Aid, &TimLen, pBcastFlag, pDtimCount, pDtimPeriod, pMessageToMe);
                    }
#endif /* P2P_SUPPORT */
	...
	*LengthVIE = 0; /* Set the length of VIE to init value 0*/
        ...
        switch(pEid->Eid)
	{
		...
		case IE_TIM:
			...
			{
#ifdef P2P_SUPPORT
				if (P2P_CLI_ON(pAd) && NdisEqualMemory(&ie_list->Bssid[0], pAd->ApCfg.ApCliTab[0].ApCliMlmeAux.Bssid, MAC_ADDR_LEN))
				{
					GetTimBit((PCHAR)pEid, pAd->ApCfg.ApCliTab[0].ApCliMlmeAux.Aid, &TimLen,
