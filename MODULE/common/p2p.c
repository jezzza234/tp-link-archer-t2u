	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	pP2PCtrl->bUsePeerChannel = USE_PEER_CHANNEL_DEFAULT;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	INT ext_ch;
	PAPCLI_STRUCT pApCliEntry = &pAd->ApCfg.ApCliTab[0];
	...
	DBGPRINT(RT_DEBUG_TRACE, ("CentralChannel = %d, Channel %d, BW = %d\n",
			pAd->CommonCfg.CentralChannel, pAd->CommonCfg.Channel,
			pApCliEntry->ApCliMlmeAux.HtCapability.HtCapInfo.ChannelWidth));

	if ( (pAd->CommonCfg.CentralChannel > pAd->CommonCfg.Channel) &&
		(pApCliEntry->ApCliMlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40))
	{
		rf_channel = pAd->CommonCfg.CentralChannel;
		rf_bw = BW_40;
		ext_ch = EXTCHA_ABOVE;
