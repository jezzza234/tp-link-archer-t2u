...
#ifdef NET_SKBUFF_DATA_USES_OFFSET // mtk patch: To fix linux nb crash issue
		pClonePacket = duplicate_pkt(pAd->net_dev, pPayload, LENGTH_802_3, pPayload+LENGTH_802_3, PayloadSize-LENGTH_802_3, 0);
#else
		...
		*/
		...
		if (P2P_GO_ON(pAd) || P2P_CLI_ON(pAd))
		{
			if (NdisEqualMemory(pAd->P2PCurrentAddress, pHeader->Addr1, MAC_ADDR_LEN) ||
				...
				NdisEqualMemory(pAd->ApCfg.ApCliTab[0].ApCliMlmeAux.Bssid, pHeader->Addr2, MAC_ADDR_LEN))
			{
