	...
	if (index < MAX_P2P_GROUP_SIZE)
	{
		...
		for (iPeerChannelIndex = 0;iPeerChannelIndex < MAX_NUM_OF_CHANNELS;iPeerChannelIndex++)
		{
			...
			/*
				...
				If use peer channel policy is TRUE and
				no connection and
				...
				use peer channel.
			*/
			if((pP2PCtrl->bUsePeerChannel == TRUE) &&
		...
		DBGPRINT(RT_DEBUG_TRACE, ("bUsePeerChannel = %d. INFRA_ON = %d. P2P_GO_ON = %d. P2P_CLI_ON = %d. PeerChannelListNum = %d. PeerChannelIs5GBand = %d\n", 
			pP2PCtrl->bUsePeerChannel, INFRA_ON(pAd), P2P_GO_ON(pAd), P2P_CLI_ON(pAd), iPeerChannelIndex, pP2pEntry->bSupport5G));
