	...
	IN HTTRANSMIT_SETTING *pTransmit)
{
	...
	*/
	...
	{
		if (pMac->TxSndgType == SNDG_TYPE_NDP  || pMac->TxSndgType == SNDG_TYPE_SOUNDING || pTxWI->TxWIeTxBF)
			pTxWI->TxWISTBC = 0;
	}
#endif /* TXBF_SUPPORT */
	...
	*/
	...
	else if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP)
	{
		if (pTxBlk->TxNDPSndgMcs >= 16)
			pTxWI->TxWINDPSndRate = 2;
		else if (pTxBlk->TxNDPSndgMcs >= 8)
			pTxWI->TxWINDPSndRate = 1;
		else
			pTxWI->TxWINDPSndRate = 0;

		pTxWI->TxWINDPSndBW = pTransmit->field.BW;
		pTxWI->TxWIiTxBF = 0;
	}
	else
	{
		...
		if (pMacEntry && (pMacEntry->mrqCnt >0) && (pMacEntry->toTxMrq == TRUE))
			pTxWI->TxWIeTxBF = ~(pTransmit->field.eTxBF);
		else
#endif	/* MFB_SUPPORT */
			pTxWI->TxWIeTxBF = pTransmit->field.eTxBF;
		pTxWI->TxWIiTxBF = pTransmit->field.iTxBF;
	}

	if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP  || pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING || pTxWI->TxWIeTxBF)
		pTxWI->TxWISTBC = 0;
#endif /* TXBF_SUPPORT */
...
#ifdef TXBF_SUPPORT
		rate_ctrl.field.iTxBF = pTxWI->TxWIiTxBF;
		rate_ctrl.field.eTxBF = pTxWI->TxWIeTxBF;
#endif /* TXBF_SUPPORT */
	...
	IN TX_BLK *pTxBlk)
{
	...
	{
		pAd->DiagStruct.TxDataCnt[pAd->DiagStruct.ArrayCurIdx]++;
		pAd->DiagStruct.TxMcsCnt[pAd->DiagStruct.ArrayCurIdx][pTxWI->TxWIMCS]++;
	}
#endif /* DBG_DIAGNOSE */
...
*/
	...
	else
	{
		...
		if (pTxWI->TxWIeTxBF || pTxWI->TxWIiTxBF)
			pTxWI->TxWISTBC = 0;
	}
