			...
			*/
				...
				(WMODE_CAP_N(pAd->CommonCfg.PhyMode) && bAllowNrate))
			{
					...
					(ie_list->vht_cap_len))
				{
					...
					pAd->CommonCfg.vht_cent_ch = vht_cent_ch_freq(pAd, pAd->MlmeAux.Channel);
...
#ifdef BEACON_RSSI
{
	Update_Rssi_Sample(pAd, &pAd->StaCfg.BeaconRssiSample, &RxWI);
	pAd->StaCfg.Rssi_DVGA_Acc =(( (pAd->StaCfg.Rssi_DVGA_Acc * 7) / 8) + (pAd->StaCfg.BeaconRssiSample.AvgRssi0 *32));
	//DBGPRINT(RT_DEBUG_ERROR, ("PeerBeacon - pAd->StaCfg.BeaconRssiSample.AvgRssi0=%d, X32=%d, pAd->StaCfg.Rssi_DVGA_Acc=%d\n", pAd->StaCfg.BeaconRssiSample.AvgRssi0, (pAd->StaCfg.BeaconRssiSample.AvgRssi0 *32), pAd->StaCfg.Rssi_DVGA_Acc));
