	...
	OUT STATE_MACHINE_FUNC Trans[]) 
{
	...
	for (i=0; i < MAX_APCLI_NUM; i++)
	{
		...
		/* timer init */
		RTMPInitTimer(pAd, &pAd->ApCfg.ApCliTab[i].ApCliMlmeAux.ApCliAssocTimer, GET_TIMER_FUNCTION(ApCliAssocTimeout), pAd, FALSE);
	}
...
/* 2008/12/17:KH modified to fix the low throughput of  AP-Client  on Big-Endian Platform<-- */
...
#ifndef RT_BIG_ENDIAN
			{
				MakeOutgoingFrame(pOutBuffer + FrameLen,            &TmpLen,
							 ...
							 pApCliEntry->ApCliMlmeAux.HtCapabilityLen,          &pApCliEntry->ApCliMlmeAux.HtCapability, 
							  END_OF_ARGS);
			}
#else
...
/* 2008/12/17:KH modified to fix the low throughput of  AP-Client  on Big-Endian Platform-->	 */
		...
		{
			if (pApCliEntry->UapsdInfo.bAPSDCapable && pApCliEntry->ApCliMlmeAux.APEdcaParm.bAPSDCapable)
			{
		...
		&HtCapability, &AddHtInfo, &HtCapabilityLen,&AddHtInfoLen,&NewExtChannelOffset, &EdcaParm, &CkipFlag))
	{
		...
		if(MAC_ADDR_EQUAL(Addr2, pApCliEntry->ApCliMlmeAux.Bssid))
		{
			...
			if(Status == MLME_SUCCESS) 
			{
				...
				pApCliEntry->ApCliMlmeAux.Aid=Aid;
