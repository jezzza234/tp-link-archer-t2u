	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Status == MLME_SUCCESS)
	{
		...
		*pCurrState = APCLI_CTRL_AUTH;
		...
		COPY_MAC_ADDR(AuthReq.Addr, pApCliEntry->ApCliMlmeAux.Bssid);
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(Status == MLME_SUCCESS)
	{
		...
		*pCurrState = APCLI_CTRL_ASSOC;
		...
		AssocParmFill(pAd, &AssocReq, pApCliEntry->ApCliMlmeAux.Bssid, pApCliEntry->ApCliMlmeAux.CapabilityInfo,
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState = APCLI_CTRL_AUTH;
	...
	COPY_MAC_ADDR(AuthReq.Addr, pApCliEntry->ApCliMlmeAux.Bssid);
	AuthReq.Alg = pApCliEntry->ApCliMlmeAux.Alg; /* Ndis802_11AuthModeOpen; */
	AuthReq.Timeout = AUTH_TIMEOUT;
	MlmeEnqueue(pAd, APCLI_AUTH_STATE_MACHINE, APCLI_MT2_MLME_AUTH_REQ,
		sizeof(MLME_AUTH_REQ_STRUCT), &AuthReq, ifIndex);
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	NdisZeroMemory(pApCliEntry->ApCliMlmeAux.Bssid, MAC_ADDR_LEN);
	pApCliEntry->ApCliMlmeAux.SsidLen = 0;
	NdisZeroMemory(pApCliEntry->ApCliMlmeAux.Ssid, MAX_LEN_OF_SSID);
	pApCliEntry->ApCliMlmeAux.Rssi = 0;
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	DisassocParmFill(pAd, &DisassocReq, pApCliEntry->ApCliMlmeAux.Bssid, REASON_DISASSOC_STA_LEAVING);
	...
	NdisZeroMemory(pApCliEntry->ApCliMlmeAux.Bssid, MAC_ADDR_LEN);
	pApCliEntry->ApCliMlmeAux.SsidLen = 0;
	NdisZeroMemory(pApCliEntry->Ssid, MAX_LEN_OF_SSID);
	pApCliEntry->ApCliMlmeAux.Rssi = 0;
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	COPY_MAC_ADDR(DeAuthFrame.Addr, pApCliEntry->ApCliMlmeAux.Bssid);
