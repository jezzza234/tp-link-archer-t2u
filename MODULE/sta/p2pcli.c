	...
	IN OUT	ADD_HT_INFO_IE 		*pAddHtInfo)
{
	...
	*/
	...
	pApCliEntry->ApCliMlmeAux.HtCapability.HtCapParm.MaxRAmpduFactor = pAd->CommonCfg.DesiredHtPhy.MaxRAmpduFactor;
	...
	IN UCHAR ifIndex)
{
	...
	do
	{
		...
		if (pMacEntry)
		{
			...
			BssTableDeleteEntry(&pAd->ScanTab,
					    /*pApCliEntry->ApCliMlmeAux.Bssid*/pAd->P2pCfg.Bssid,
			...
			if (pApCliEntry->bAutoTxRateSwitch == FALSE)
			{
				pMacEntry->bAutoTxRateSwitch = FALSE;
				/* If the legacy mode is set, overwrite the transmit setting of this entry.  	 */		
				RTMPUpdateLegacyTxSetting((UCHAR)pApCliEntry->DesiredTransmitSetting.field.FixedTxMode, pMacEntry);	
			}
			else
			{
				UCHAR TableSize = 0;
	...
	if ((result == TRUE) && 
		(pApCliEntry->WscControl.WscConfMode == WSC_ENROLLEE) &&
		(pApCliEntry->WscControl.bWscTrigger == TRUE))
	{
		pApCliEntry->WscControl.WscState = WSC_STATE_LINK_UP;
		pApCliEntry->WscControl.WscStatus = WSC_STATE_LINK_UP;
		pApCliEntry->WscControl.WscConfStatus = WSC_SCSTATE_UNCONFIGURED;
