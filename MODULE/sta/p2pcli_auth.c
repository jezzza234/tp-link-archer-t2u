	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	pCurrState = &pApCliEntry->AuthCurrState;
	...
	else if(MlmeAuthReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr, &Timeout, &Alg))
	{
		...
		ApCliMgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, Addr, pApCliEntry->ApCliMlmeAux.Bssid, ifIndex);
		...
		RTMPSetTimer(&pApCliEntry->ApCliMlmeAux.ApCliAuthTimer, AUTH_TIMEOUT);
...
/*	UCHAR           ChlgText[CIPHER_TEXT_LEN]; */
	...
	{
		if(MAC_ADDR_EQUAL(pApCliEntry->ApCliMlmeAux.Bssid, Addr2) && Seq == 2)
		{
			...
			{
				if(pApCliEntry->ApCliMlmeAux.Alg == Ndis802_11AuthModeOpen)
				{
					*pCurrState = APCLI_AUTH_REQ_IDLE;
				...
				{
					PCIPHER_KEY  pKey;	
					UINT	default_key = pApCliEntry->DefaultKeyId;
					...
					ApCliMgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, Addr2, pApCliEntry->ApCliMlmeAux.Bssid, ifIndex);
					AuthHdr.FC.Wep = 1;
