...
static RTMP_REG_PAIR MT76x0_BBP_Init_Tab[] = {
	...
	*/
	...
	{RXO_R13, 0x00000092},	/* 192->092 ,2934 bit8 default 0 to avoid IOT with QCA AR9880 */
	{RXO_R14, 0x00060612},
	{RXO_R15, 0xC8321B18},
	{RXO_R16, 0x0000001E},
	{RXO_R17, 0x00000000},
...
*/
	...
	*/
	...
	{RF_BANK5,	RF_R02, 0x0C}, /* 0x1D: 5G+2G+BT(MT7650E),  0x0C: 5G+2G(MT7610U), 0x00: 5G only (MT7610E) */
		...
		if (((rf_phy_mode | rf_bw) & MT76x0_BPP_SWITCH_Tab[Index].BwBand) == (rf_phy_mode | rf_bw))
		{
			if ((MT76x0_BPP_SWITCH_Tab[Index].RegDate.Register == AGC1_R8))
			{
				...
				if (Channel > 14)
				{
					...
					pAd->chipCap.init_vga_gain_5G = eLNAgain;
				#endif
				}
				else
				{
				...
				#ifdef CONFIG_STA_SUPPORT
					pAd->chipCap.init_vga_gain_2G = eLNAgain;
				#endif
				}
...
INT32 mt76x0_get_current_temp(
	IN RTMP_ADAPTER 	*pAd)
{
	MT76x0_TempSensor(pAd);
	
	return pAd->chipCap.NowTemperature;
}
	...
	IN RTMP_ADAPTER 	*pAd)
{
	...
	RTMP_IO_WRITE32(pAd, AMPDU_MAX_LEN_20M1S, 0xbaa99887);
	RTMP_IO_WRITE32(pAd, EDCA_AC0_CFG, pAd->default_edca_ac0);
	...
	IN RTMP_ADAPTER 	*pAd)
{
	...
	Ac0Cfg.word = pAd->default_edca_ac0;
	MAC_TABLE_ENTRY *pEntry;
	pEntry = &pAd->MacTab.Content[BSSID_WCID];
	switch ( pEntry->HTPhyMode.field.MCS )
	{
		case 0:
		case 1:
			Ac0Cfg.field.AcTxop = 10;
	...
	/* Default A-MPDU length */
	RTMP_IO_WRITE32(pAd, AMPDU_MAX_LEN_20M1S, 0x77663333);
	...
	IN RTMP_ADAPTER 	*pAd)
{
	...
	switch ( pEntry->HTPhyMode.field.MCS )
	{
		case 0:
			...
			Ac0Cfg.field.Cwmin = 8;
			RTMP_IO_WRITE32(pAd, EDCA_AC0_CFG, Ac0Cfg.word);
			break;
		case 1:
		case 2:
			Ac0Cfg.field.AcTxop = 0;
			Ac0Cfg.field.Cwmin = 7;
			RTMP_IO_WRITE32(pAd, EDCA_AC0_CFG, Ac0Cfg.word);
			break;
		case 3:
		case 4:
	...
	RTMP_IO_WRITE32(pAd, AMPDU_MAX_LEN_20M1S, 0x55544444);
...
*/
	...
	pChipOps->ChipGetCurrentTemp = mt76x0_get_current_temp;
	...
	IN RTMP_ADAPTER 	*pAd)
{
	...
	RTMP_CHIP_CAP *pChipCap = &pAd->chipCap;
	UINT32 reg_val = 0, init_vga = 0;
	INT32 rssi= 0;
	rssi = pAd->StaCfg.RssiSample.AvgRssi0 - pAd->BbpRssiToDbmDelta;

#ifdef BEACON_RSSI
	Rssi_DVGA = (pAd->StaCfg.Rssi_DVGA_Acc>>8);
	DBGPRINT(RT_DEBUG_ERROR, ("@@@ BEFORE %s(): rssi=%d, Rssi_DVGA=%d\n", __FUNCTION__, rssi, Rssi_DVGA));
	/*if(pAd->CommonCfg.BBPCurrentBW == BW_80){
		if (Rssi_DVGA > -60) {
			Rssi_DVGA -= 8;
		} else if (Rssi_DVGA > -70) {
			Rssi_DVGA -= 4;	
		}
	}
	else if(pAd->CommonCfg.BBPCurrentBW == BW_40){
    ...
    if (pAd->CommonCfg.CentralChannel > 14){
	    ...
	    if (rssi > -46){  // optimum value is -48 but change to -46 due to rssi variation by CCK packet
			reg_val = 0x06061eb0;
			reg_val &= 0xFFFF80FF;
			init_vga -= 48;
			reg_val |= (init_vga << 8);
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s(): RSSI=%d, BBP 2320=0x%x\n", __FUNCTION__, rssi, reg_val));
}
